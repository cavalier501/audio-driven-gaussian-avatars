...
def render_set(dataset : ModelParams, name, iteration, views, gaussians, pipeline, background, render_mesh,render_mesh_background:str="no_background"):
    # # # date:2025.04.17 ⬇️
    # # # 为了作图，调高不透明度
    # gaussians._opacity=torch.zeros_like(gaussians._opacity)+1
    # # gaussians._features_dc=torch.rand_like(gaussians._features_dc)
    # gaussians._scaling=gaussians._scaling*3
    ...
    # TODO
    render_full_rgb_path = iter_path / "renders_full_rgb_result"
    ...
    try:
        ...
        # 25.4.10 将fps调整为30
        # TODO
        os.system(f"ffmpeg -y -framerate 30 -f image2 -pattern_type glob -i '{render_full_rgb_path}/*.png' -c:v libx264 -pix_fmt yuv420p {iter_path}/renders_full_rgb_result.mp4")
...
if __name__ == "__main__":
    """
    整体过程、相关参数配置与render.py一致
    增加了对不同人脸分割部分的单独查看功能，主要用于牙齿部分几何补全策略的消融实验结果查看
    """
    ...
    # parser.set_defaults(model_path=".//GaussianAvatarsoutput/UNION10EMOEXP_306_dataset_redivide")
    ...
    base_name="D1_angry1_f_111" # ["D1_angry1_f_111","D1_angry2_f_120","D1_bazongnew_f_432","D1_ted1_f_643"]
    args.result_save_path=f"output/audio_check_demo_with_teeth"
    ...
    args.vert_meshes_path=f"/home/zh/master_thesis_supplementary/UniTalker/test_data_for_gsa_306_v1/{base_name}.pt"
    num_of_frames=torch.load(args.vert_meshes_path).shape[0]
    # print(torch.load(args.vert_meshes_path).shape)
    # sys.exit()
    # ↓基于flame参数时，flame参数文件
    args.flame_path=".//GaussianAvatarsoutput/UNION10EMOEXP_306_dataset_redivide/point_cloud/iteration_600000/flame_param.npz"
    # ↓高斯属性文件（PLY文件）
