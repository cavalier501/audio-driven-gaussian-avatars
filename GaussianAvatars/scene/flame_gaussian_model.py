...
class FlameGaussianModel(GaussianModel):
    def __setattr__(self, name, value):
        ...
        # 监视某个变量，比如 `_xyz`
        if name == "_xyz":
            # print(f"变量 `_xyz` 发生变化")
            # input("按回车继续执行...")
            # pdb.set_trace()
            # breakpoint()
            pass
        super().__setattr__(name, value)  # 允许正常赋值 
    ...
    def load_ply(self, path, **kwargs):
        super().load_ply(path)
        # 在官方的render用例中。kwargs只有一个键"has_target"
        # kwargs["has_target"] = False
        ...
        if not kwargs['has_target']:
            ...
            if "flame_path" in kwargs and kwargs['flame_path'] is not None:
                npz_path = Path(kwargs['flame_path'])
            else:
    ...
    def load_vert_meshes(self,
            vert_meshes_path:str,
            ply_path:str="./output/UNION10EMOEXP_306_eval_600k/point_cloud/iteration_600000/point_cloud.ply", # gaussian相关信息
        ):
        # date:2025.03.10
        # note:
        # 从flame拓扑的mesh（[frame,5143,3]）中加载顶点信息
        # 注意，在函数load_meshes中，"mesh"似乎特指flame参数
        # 此函数中的meshes表示每个顶点的坐标
        # 同时，额外引入一个load_vert_meshes_flag。用于标记是否加载了逐个顶点信息
        # 在flame_model.flame.FlameHead.forward()中，verts_cano为不含lbs、translation的flame mesh
        super().load_ply(ply_path)
        self.load_vert_meshes_flag=True
        vert_meshes=torch.load(vert_meshes_path).cuda() # [frame,5143,3]
        self.vert_meshes   = vert_meshes
        self.num_timesteps = vert_meshes.shape[0]
        return
    ...
    def select_mesh_by_timestep(self, timestep:int, original=False):
        # date:2025.03.10
        # note:
        # 补充直接基于self.vert_meshes实现flame mesh
        # date:2025.03.12
        # 补充：如果timestep大于帧数据的长度（如数据格式中出现了这种问题），
        # 则取最后一帧，即输出视频的最后一些帧保持静止
        self.timestep = timestep
        if (hasattr(self, 'load_vert_meshes_flag') and self.load_vert_meshes_flag)==False:

            flame_param = self.flame_param_orig if original and self.flame_param_orig != None else self.flame_param
            timestep=min(timestep,flame_param['expr'].shape[0]-1)
            verts, verts_cano = self.flame_model(
            ...
            timestep=min(timestep,self.vert_meshes.shape[0]-1)
            verts,verts_cano=self.vert_meshes[[timestep]].cuda(),self.vert_meshes[[timestep]].cuda()
        self.update_mesh_properties(verts, verts_cano)
    ...
    def save_deformed_ply(self,path,):
        # date:2025.04.17
        # note:
        # 保存被形变后(跟随绑定mesh被驱动后)的ply
        import os
