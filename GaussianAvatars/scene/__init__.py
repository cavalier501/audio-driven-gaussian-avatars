...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : Union[GaussianModel, FlameGaussianModel], load_iteration=None, shuffle=True, resolution_scales=[1.0]):
        ...
        if self.loaded_iter:
            # TODO
            if args.load_vert_meshes_flag:
                self.gaussians.load_vert_meshes(
                    vert_meshes_path = args.vert_meshes_path,
                ...
                #         self.ply_path,
                #         has_target=args.target_path != "", # 在render.py的官方测试用例中，args.target_path=""，即has_target=False
                #     )
                # except:
                self.gaussians.load_ply(
                    os.path.join(self.model_path,
                                ...
                                "point_cloud.ply"),
                    has_target=args.target_path != "", # 在render.py的官方测试用例中，args.target_path=""，即has_target=False
                    flame_path=None if hasattr(args, "flame_path") == False else args.flame_path,
                )
        else:
    ...
    def save_best_ckpt(self, iteration):
        point_cloud_path = os.path.join(self.model_path, "point_cloud/best_ckpt")
        if os.path.exists(point_cloud_path)==False:
            os.makedirs(point_cloud_path)
        for old_best_ckpt in os.listdir(point_cloud_path):
            if old_best_ckpt.endswith(".ply") or old_best_ckpt.endswith(".npz"):
                os.remove(os.path.join(point_cloud_path, old_best_ckpt))
        self.gaussians.save_ply(os.path.join(point_cloud_path, f"point_cloud_iter_{iteration}.ply"))
