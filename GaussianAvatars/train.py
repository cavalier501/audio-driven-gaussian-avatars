...
note:
修改保存ckpt逻辑：
修改前：按照interation保存ckpt
修改后：出现测试集最佳结果时保存ckpt，但为了避免保存过于频繁，iter为interval整数倍时才进行最佳ckpt的检测
    在training_report中实现，评价标准：val数据集 l1 loss
"""
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    testing_iterations=[x for x in testing_iterations if x>=first_iter]
    testing_iterations.sort()
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        with torch.no_grad():
            ...
            # Log and save
            loss_for_this_iter=training_report(tb_writer, iteration, losses, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, (pipe, background))
            if iteration==first_iter:
                best_loss=loss_for_this_iter
                print(best_loss)
                print("[ITER {}] Saving bset Gaussians on testing_iterations".format(iteration))
                scene.save_best_ckpt(iteration)
            elif iteration in testing_iterations:
                if loss_for_this_iter<best_loss:
                    best_loss=loss_for_this_iter
                    print("[ITER {}] Saving bset Gaussians on testing_iterations".format(iteration))
                    scene.save_best_ckpt(iteration)
            if (iteration in saving_iterations):
                print("[ITER {}] Saving Gaussians on saving_iterations".format(iteration))
            ...
            if (iteration in checkpoint_iterations):
                print("[ITER {}] Saving Checkpoint on checkpoint_iterations".format(iteration))
...
def training_report(tb_writer, iteration, losses, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs):
    ...
    # NOTE 25.3.24 基于testing_iterations进行最佳ckpt的检测
    if iteration in testing_iterations:
        ...
        torch.cuda.empty_cache()
        loss_for_this_iter= l1_test
        return loss_for_this_iter
    else:
        return None
...
if __name__ == "__main__":
    ...
    # # 相关config文件内写死
    # # args.source_path      = "data/UNION10_306_EMO1234EXP234589_v16_DS2-0.5x_lmkSTAR_teethV3_SMOOTH_offsetS_whiteBg_maskBelowLine"
    # # args.source_path      = "data/UNION10_306_EMO1234EXP234589_v16_DS2-0.5x_lmkSTAR_teethV3_SMOOTH_offsetS_whiteBg_maskBelowLine"
    # args.source_path      = "/home/zh/master_thesis_supplementary/GaussianAvatars/data/UNION10_306_EMO1234EXP234589_v16_DS2-0.5x_lmkSTAR_teethV3_SMOOTH_offsetS_whiteBg_maskBelowLine"

    # # args.model_path       = "output/UNION10EMOEXP_306_eval_test_for_ckpt_strategy"
    # args.model_path       = "output/UNION10EMOEXP_306_test_0428"
    # args.eval             = True
    # args.bind_to_mesh     = True
    # args.white_background = True
    # args.port             = 60001
    # args.start_checkpoint = "output/UNION10EMOEXP_306_eval_600k/chkpnt600000.pth"
    # test_iterations决定了进行ckpt的频率
    # args.test_iterations=[60000, 120000, 180000, 240000, 300000, 360000, 420000, 480000, 540000, 600000]
    args.test_iterations=list(range(360000, 600001, 12000)) # 20次评测，耗时大概30分钟
    args.save_iterations=[60000, 120000, 180000, 240000, 300000, 360000, 420000, 480000, 540000, 600000]
    args.checkpoint_iterations=[60000, 120000, 180000, 240000, 300000, 360000, 420000, 480000, 540000, 600000]
