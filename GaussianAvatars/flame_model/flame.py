...
class FlameHead(nn.Module):
    ...
    def add_teeth(self):
        ...
        # 注：官方对J_regressor形状的注释似乎有问题，应该是[5,5023]->[5,5143]
    ...
    def add_teeth_to_mesh(self,vert_meshes:torch.Tensor):
        """
        date       : 2025.03.12
        input      : 
        vert_meshes: [bs,5023,3]
        output     : 
        vert_meshes: [bs,5143,3]
        description: 
        对具体的flame mesh顶点（5023顶点）进行add_teeth操作
        注意，不会对flame模型进行修改，只是对输入的mesh进行修改
        """
        ...
        v_lip_upper = vert_meshes[:,vid_lip_outside_ring_upper,:]
        v_lip_lower = vert_meshes[:,vid_lip_outside_ring_lower,:]

        

        # construct vertices for teeth
        mean_dist = (v_lip_upper - v_lip_lower).norm(dim=-1, keepdim=True).mean(dim=1).squeeze(-1) # [bs,1]
        v_teeth_middle = (v_lip_upper + v_lip_lower) / 2 # [bs,m,3]
        v_teeth_middle[:,:, 1] = v_teeth_middle[:,:, [1]].mean(dim=1, keepdim=True).squeeze(-1)
        # v_teeth_middle[:, 2] -= mean_dist * 2.5  # how far the teeth are from the lips
        # v_teeth_middle[:, 2] -= mean_dist * 2  # how far the teeth are from the lips
        v_teeth_middle[:,:, 2] -= mean_dist.unsqueeze(1) * 1.5  # how far the teeth are from the lips

        # upper, front
        mean_dist_tensor=torch.stack([
            torch.zeros_like(mean_dist),mean_dist,torch.zeros_like(mean_dist)
        ],dim=1).unsqueeze(1) # [bs,1,3]
        v_teeth_upper_edge = v_teeth_middle.clone() + mean_dist_tensor*0.1
        v_teeth_upper_root = v_teeth_upper_edge + mean_dist_tensor * 2  # scale the height of teeth

        # lower, front
        v_teeth_lower_edge = v_teeth_middle.clone() - mean_dist_tensor*0.1
        v_teeth_lower_edge -= mean_dist_tensor * 0.4  # slightly move the lower teeth to the back
        v_teeth_lower_root = v_teeth_lower_edge - mean_dist_tensor * 2  # scale the height of teeth

        # thickness = mean_dist * 0.5
        thickness = mean_dist.unsqueeze(1) * 1. # [bs,1]
        # upper, back
        v_teeth_upper_root_back = v_teeth_upper_root.clone()
        v_teeth_upper_edge_back = v_teeth_upper_edge.clone()
        v_teeth_upper_root_back[:,:, 2] -= thickness  # how thick the teeth are # [bs,15,3]
        v_teeth_upper_edge_back[:,:, 2] -= thickness  # how thick the teeth are
        ...
        # concatenate to v_template
        num_verts_orig = vert_meshes.shape[1]
        v_teeth = torch.cat([
            v_teeth_upper_root,  # num_verts_orig + 0-14 
            v_teeth_lower_root,  # num_verts_orig + 15-29
            v_teeth_upper_edge,  # num_verts_orig + 30-44
        ...
        num_verts_teeth = v_teeth.shape[1]
        vert_meshes = torch.cat([vert_meshes, v_teeth], dim=1)
        return vert_meshes
