...
date:2025.01.15
note:
测试：对于unseen subject，不同id embedding设置策略的效果
预期文件形式：
1 通过args指定id embedding strategy
2 通过额外函数完成LVE评测
id embedding策略：
1 each: 复用原始代码的策略，每种数据集使用一个训练集中特性subject的id embedding
2 common: 复用原始代码的策略，使用pivot id embedding
3 average_1：使用所有数据集、所有subject的平均id embedding
4 average_2：使用特定数据集、所有subject的平均id embedding
5 nearest：实现重点，将训练集中的id embeddings当作codebook，对于unseen subject，找到最近的id embedding
"""
...
def main():
    args = get_parser()
    allowed_id_strategies = ['each', 'common', 'average_1', 'average_2', 'nearest']
    assert args.condition_id in allowed_id_strategies, f"condition_id should be one of {allowed_id_strategies}"
