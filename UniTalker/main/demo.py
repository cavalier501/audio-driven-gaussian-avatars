    ...
    with torch.no_grad():
        for wav_f in wav_f_names:
            ...
            for dataset_name in demo_dataset_name_list:
                ...
                if dataset_name=="D1":
                    os.makedirs("./audio_driven_D1_deformation", exist_ok=True)
                    out_ndarray=out.cpu().numpy().reshape(-1,5023,3)
                    wav_prefix = wav_f.split(".")[0]
                    np.save(
                        f"./audio_driven_D1_deformation/{dataset_name}_{wav_prefix}_f_{out_ndarray.shape[0]}",
                        out_ndarray,
                    )
